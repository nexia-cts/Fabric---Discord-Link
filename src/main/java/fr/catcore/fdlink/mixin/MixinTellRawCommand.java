...
import fr.catcore.fdlink.api.minecraft.compat.CommandMessageCompat;
...
//    }
    ...
    /**
     * @author Cat Core
     * @reason Injecting in lambda doesn't seem to work
     */
    @Overwrite
        ...
        dispatcher.register((LiteralArgumentBuilder)((LiteralArgumentBuilder) CommandManager.literal("tellraw").requires((serverCommandSource) -> {
            ...
            if (commandContext.getInput().replace("/tellraw ", "").startsWith("@a") || commandContext.getInput().replace("tellraw ", "").startsWith("@a")) {
                try {
                    ...
                    FDLink.getMessageSender().sendMessage(new CommandMessageCompat(author.getString(), text.getString(), "tellraw"));
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
