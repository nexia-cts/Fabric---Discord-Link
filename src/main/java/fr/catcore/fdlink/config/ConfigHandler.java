...
import fr.catcore.fdlink.api.config.MessageConfig;
...
public class ConfigHandler {
    ...
    private static final Logger LOGGER = Logger.getLogger("FDLink-ConfigHandler");
    ...
    public static ConfigHolder getConfig() {
        ...
        if (OLD_CONFIG_FILE.exists()) {
            ...
            try (InputStreamReader fileReader = new InputStreamReader(Files.newInputStream(OLD_CONFIG_FILE.toPath()), StandardCharsets.UTF_8)) {
            ...
            if (NEW_MAIN_FILE.exists()) {
                ...
                try (InputStreamReader mainFileReader = new InputStreamReader(Files.newInputStream(NEW_MAIN_FILE.toPath()), StandardCharsets.UTF_8)) {
                    ...
                    if (version < 2) configHolder = parseConfig(mainObject);
                        ...
                        if (MESSAGE_CONFIG.exists()) {
                            try (InputStreamReader messageFileReader = new InputStreamReader(Files.newInputStream(MESSAGE_CONFIG.toPath()), StandardCharsets.UTF_8)) {
                            ...
                            try (OutputStreamWriter messageFileWriter = new OutputStreamWriter(Files.newOutputStream(MESSAGE_CONFIG.toPath()), StandardCharsets.UTF_8)) {
    ...
    private static void saveMainConfig(JsonObject jsonObject) {
        ...
        try (OutputStreamWriter writer = new OutputStreamWriter(Files.newOutputStream(NEW_MAIN_FILE.toPath()), StandardCharsets.UTF_8)) {
    ...
    private static void saveMessageConfig(JsonObject jsonObject) {
        ...
        try (OutputStreamWriter writer = new OutputStreamWriter(Files.newOutputStream(MESSAGE_CONFIG.toPath()), StandardCharsets.UTF_8)) {
